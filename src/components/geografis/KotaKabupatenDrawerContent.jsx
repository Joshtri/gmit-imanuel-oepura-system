  ...
  if (!data?.length) {
    return (
      <div className="text-center py-12">
        ...
        <p className="text-sm text-gray-500 mb-2">Belum ada kota/kabupaten</p>
        <p className="text-xs text-gray-400">untuk provinsi {provinsiName}</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-3">
      ...
      {data.map((kab, index) => (
        <div
          key={kab.id}
          className="border border-gray-200 p-4 rounded-lg bg-white hover:bg-gray-50 transition-colors shadow-sm"
        >
          <div className="flex items-center justify-between">
            <div>
              ...
              <div className="text-xs text-gray-500 mt-1">ID: {kab.id}</div>
