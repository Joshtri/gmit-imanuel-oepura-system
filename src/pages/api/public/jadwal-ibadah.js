...
async function handler(req, res) {
  try {
    ...
    const formattedSchedules = jadwalIbadah.map(jadwal => ({
      ...
      date: jadwal.tanggal ? new Date(jadwal.tanggal).toLocaleDateString('id-ID', {
        weekday: 'long',
        year: 'numeric', 
        month: 'long',
        day: 'numeric'
      }) : null,
      time: jadwal.waktuMulai ? (typeof jadwal.waktuMulai === 'string' ? jadwal.waktuMulai.substring(0, 5) : jadwal.waktuMulai.toLocaleTimeString('id-ID', { hour: '2-digit', minute: '2-digit' })) : 'Belum ditentukan',
